...
library generalFunctions {
    ...
    function requireOwner() private view {
        require(msg.sender == this.owner, "You are not the user");
    }
...
library userFunctions {
    ...
    function findVaultUser(
        ...
        for (uint64 i = 0; i < vaults[vaultIndex].numUsers; i++) {
            if (
                generalFunctions.compareStrings(email, vaults[vaultIndex].vaultUsers[i].email) ==
                true
            ) {
                return true;
            } else {
    ...
    function addUserData(
        ...
        generalFunctions.requireOwner();
        require(
            generalFunctions.compareStrings(users[email].hashPassPhrase, hashPass) == true,
            "Invalid Password"
        );
        require(
            generalFunctions.compareStrings(users[email].email, "") == false,
            "User Not Found"
        );
        userFunctions.requireObjOwner(email, users[email].email);
    ...
    function getAllTxnHashes(
        ...
        require(
            generalFunctions.compareStrings(users[_email].email, "") == false,
            "User Not Found"
        );
...
contract OnePass {
    ...
    function createVault(
        ...
        if (size > 0) {
            for (uint8 i = 0; i < size; i++) {
                CustomFields storage tempCustomField = customFields[
                    vaultKeyHash
                ];
    ...
    function getUserEncVaultKey(
        ...
        for (uint64 i; i < vaults[vaultIndex].numUsers; i++) {
            if (
                generalFunctions.compareStrings(vaults[vaultIndex].vaultUsers[i].email, email) ==
                true
            ) {
    ...
    //             if(generalFunctions.compareStrings(email, vaults[i].vaultUsers[j].email) == true){
    ...
    function addVaultUser(
        ...
        generalFunctions.requireOwner();
        require(
            generalFunctions.compareStrings(users[addUserEmail].email, "") == false,
            "User Not Found"
        );
        require(
            generalFunctions.compareStrings(users[userEmail].hashPassPhrase, hashPass) == true,
            "Invalid Password"
        );
        require(
            userFunctions.findVaultUser(addUserEmail, vaultIndex) == false,
            "User Already Assigned"
        );
        userFunctions.requireObjOwner(userEmail, vaults[vaultIndex].owner);
    ...
    function removeVaultUser(
        ...
        generalFunctions.requireOwner();
        userFunctions.requireObjOwner(email, vaults[vaultIndex].owner);
        require(
            generalFunctions.compareStrings(users[email].email, "") == false,
            "User Not Found"
        );
        ...
        for (uint64 i = 0; i < tempUser.numVaults; i++) {
            if (
                generalFunctions.compareStrings(
        ...
        for (uint64 i = 0; i < tempVault.numUsers; i++) {
            if (
                generalFunctions.compareStrings(
                    ...
                    vaults[vaultIndex].vaultUsers[userIndex].email
                ) == true
            ) {
    ...
    function removeVault(
        ...
        generalFunctions.requireOwner();
        require(
            generalFunctions.compareStrings(users[userEmail].email, "") == false,
            "User Not Found"
        );
        require(
            generalFunctions.compareStrings(users[userEmail].hashPassPhrase, hashPass) == true,
        ...
        Vault storage tempVault = vaults[vaultIndex];
        for (uint64 i = 0; i < tempVault.numUsers; i++) {
            if (generalFunctions.compareStrings(tempVault.vaultUsers[i].email, "") == false) {
    ...
    function addVaultLogin(
        ...
        require(
            userFunctions.findVaultUser(email, vaultIndex) == true,
            "User not authorized"
        );
    ...
    function removeVaultLogin(
        ...
        userFunctions.requireObjOwner(email, vaults[vaultIndex].logins[loginIndex].owner);
    ...
    // ) public returns (bool success) {
    //     generalFunctions.requireOwner();
    //     require(generalFunctions.compareStrings(users[userEmail].email, "") == false, "User Not Found");
    //     require(generalFunctions.compareStrings(users[userEmail].hashPassPhrase, hashPass) == true, "Invalid Password");
    //     userFunctions.requireObjOwner(userEmail, vaults[vaultIndex].owner);
    ...
    // ) public returns (bool success) {
    //     generalFunctions.requireOwner();
    //     require(generalFunctions.compareStrings(users[email].email, "") == false, "User Not Found");
    //     require(generalFunctions.compareStrings(users[email].hashPassPhrase, hashPass) == true, "Invalid Password");
    //     userFunctions.requireObjOwner(email, vaults[vaultIndex].logins[loginIndex].owner);
